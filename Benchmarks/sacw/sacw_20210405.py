...
parser.add_argument('--num_steps', type=int, default=100098, metavar='N',
...
with torch.no_grad():
    ...
    encoding_power=[]
    ttpower=[]
    request1=np.zeros((MAX_EP_STEP,num_of_bitrate))
    while not done:
        #print(state)
        ...
        ttpower.append(l3)
        temp=action[1:]
        for transcoding_transformer in range(int(state[1])+1):
