...
class SAC(nn.Module):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.critic = QNetwork(num_inputs, action_space.shape[0], args.hidden_size,args.num_header,args.num_layer).to(device=self.device)
        self.critic_optim = Adam(self.critic.parameters(), lr=args.lr)

        self.critic_target = QNetwork(num_inputs, action_space.shape[0], args.hidden_size,args.num_header,args.num_layer).to(self.device)
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        min_qf_pi = torch.min(qf1_pi, qf2_pi) # the critic value
